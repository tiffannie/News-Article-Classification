...
TESTDF.drop(columns = ['Author'], inplace= True)

final_headlines = []


for a in TESTDF['Headlines']:
    if len(a) > 1000:
        final_headlines.append(a)

    
df1 = pd.read_csv('News_new5.csv')

for a in df1['Headlines']:    
    a = str(a)
    if len(a) > 1000:
        final_headlines.append(a)
...
#Prepare the corpus
corpus = []
for i in range(0, len(final_headlines)):    
    text = final_headlines[i]
...
for a in newdf1['Headlines']:
    textwords = nltk.word_tokenize(a)
...
#        d=lemmatizer.lemmatize(d)
    ...
    te.append(TreebankWordDetokenizer().detokenize(textcleantokens))
    


#Cleaned text
newdf= pd.DataFrame(te, columns = ['Clean'])
...
corpus = []
for i in range(0, len(newdf['Clean'])):    
    text = newdf['Clean'][i]
    ##Convert to list from string
    text = text.split()
    text = " ".join(text)
